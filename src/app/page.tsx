...
function createParticles(){
  ...
  for(let i = 0; i < count; i++){
    ...
    particles.push({
      ...
      color: Math.random() > 0.5 ? '30,107,50' : '141,198,63',
...
window.addEventListener('scroll', () => {
  ...
  return (
    <div style={{ background: 'var(--white)', color: 'var(--ink)' }}>
      <style dangerouslySetInnerHTML={{ __html: `
*,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
:root{
  --g1:#1E6B32;--g2:#155226;--g3:#8DC63F;--g4:#A8D94F;
  --g-soft:#EBF5EC;--g-lime:#F2FAE8;--g-mid:#C3E6C8;
  --ink:#0A0F0B;--ink2:#374039;--ink3:#6B7A6E;
  --white:#FFFFFF;--cream:#F8FAF8;
  --border:rgba(0,0,0,0.07);--border-g:rgba(30,107,50,0.18);
  ...
  --font-serif:'Google Sans Flex','Google Sans','sans-serif';
}
html{scroll-behavior:smooth}
body{font-family:'Google Sans Flex','Google Sans','sans-serif';background:var(--white);color:var(--ink);overflow-x:hidden;line-height:1.6}
...
.nav-logo{font-family:var(--font-display);font-size:22px;font-weight:900;color:var(--ink);letter-spacing:-.04em;text-decoration:none}
.nav-logo span{color:var(--g1)}
.nav-links{display:flex;gap:28px;justify-content:center;width:100%;}
...
.btn-pill{
  padding:9px 22px;border-radius:100px;font-size:14px;font-weight:500;
...
.btn-dark:hover{background:#1a2a1d;transform:translateY(-1px)}
.btn-ghost{background:transparent;color:var(--ink2);border:0.5px solid var(--border)}
.btn-ghost:hover{background:var(--cream)}

/* ══════════════════════════════
   HERO — clean white, Antigravity vibe
══════════════════════════════ */
.hero{
...
.hero-dot{width:6px;height:6px;border-radius:50%;background:var(--g3);animation:pulse 2s ease-in-out infinite}
...
.hero-h1{
  ...
  margin-bottom:28px;min-height:2.2em;
}
.hero-h1 .accent{color:var(--g1)}
.cursor-blink{
...
.hero-sub{
  font-size:clamp(16px,2vw,20px);color:var(--ink3);
...
.btn-hero-dark:hover{background:var(--g1);transform:translateY(-2px)}
.btn-hero-light{
  display:inline-flex;align-items:center;gap:8px;
...
.btn-hero-light:hover{background:var(--g-soft);border-color:var(--border-g);color:var(--g1)}
.hero-stats{
  display:flex;align-items:center;justify-content:center;gap:40px;flex-wrap:wrap;
  opacity:0;animation:fadeUp .8s .7s ease forwards;
}
.h-stat{text-align:center}
.h-stat-val{font-family:var(--font-display);font-size:26px;font-weight:900;color:var(--ink);letter-spacing:-.04em}
.h-stat-label{font-size:12px;color:var(--ink3);margin-top:2px}
.h-stat-div{width:1px;height:32px;background:var(--border)}
...
.hec:hover{transform:translateY(-3px);box-shadow:0 8px 32px rgba(0,0,0,0.1)}
.hec-send:hover{border-color:rgba(0,0,0,0.15)}
.hec-create:hover{border-color:var(--border-g)}
.hec-icon{
...
.hec-icon-green{background:var(--g1) !important}
.hec-body{flex:1;text-align:left}
.hec-title{font-family:var(--font-display);font-size:15px;font-weight:700;color:var(--ink);letter-spacing:-.02em;margin-bottom:2px}
.hec-desc{font-size:12px;color:var(--ink3);line-height:1.4}
.hec-arrow{font-size:16px;color:var(--ink3);flex-shrink:0;transition:transform .2s}
.hec:hover .hec-arrow{transform:translateX(4px);color:var(--ink)}
.hec-create:hover .hec-arrow{color:var(--g1)}

/* ══════════════════════════════
   SECTION 2 — Deep atmospheric
══════════════════════════════ */
.deep-section{
  background:#040D06;
  padding:100px 5%;
...
.glow-1{width:500px;height:500px;background:#1E6B32;top:-100px;left:-100px}
.glow-2{width:400px;height:400px;background:#8DC63F;bottom:-100px;right:10%;opacity:.2}
...
.deep-h2{
  ...
  color:#fff;letter-spacing:-.05em;line-height:1.05;
  margin-bottom:16px;
}
.deep-h2 em{color:var(--g3);font-style:normal}
.deep-sub{font-size:17px;color:rgba(255,255,255,.5);max-width:440px;line-height:1.7;margin-bottom:60px}
.float-grid{
...
.gc-icon{
  ...
  background:rgba(141,198,63,.15);border:0.5px solid rgba(141,198,63,.25);
  display:flex;align-items:center;justify-content:center;
  font-size:22px;color:var(--g3);margin-bottom:18px;
}
.gc-title{font-family:var(--font-display);font-size:18px;font-weight:800;color:#fff;letter-spacing:-.03em;margin-bottom:8px}
.gc-desc{font-size:14px;color:rgba(255,255,255,.5);line-height:1.65}

/* ══════════════════════════════
   SECTION 3 — Floating phones
══════════════════════════════ */
.phones-section{
  background:#040D06;
...
.pb-glow1{width:600px;height:400px;background:#1E6B32;opacity:.2;top:0;left:50%;transform:translateX(-50%)}
.phones-inner{
...
.fp-secondary{border:10px solid #2a2a2a;outline:1.5px solid #444;box-shadow:0 0 0 1.5px #555,inset 0 0 0 1px #111,0 30px 60px rgba(0,0,0,.7);border:8px solid #1a1a1a;box-shadow:0 0 0 1px #333,0 30px 60px rgba(0,0,0,.7),inset 0 0 0 1px #444;
  background:#081209;
  border:0.5px solid rgba(141,198,63,.2);
...
.fp-screen{background:#050E07;border-radius:32px;overflow:hidden}
...
.fp-logo span{color:var(--g3)}
.fp-body{padding:0 14px 20px}
.fp-card{background:var(--g1);border-radius:16px;padding:16px 14px;margin-bottom:12px}
.fp-card-lbl{font-size:9px;color:rgba(255,255,255,.55);margin-bottom:5px;text-transform:uppercase;letter-spacing:.07em;font-weight:500}
.fp-amount{font-family:var(--font-display);font-size:28px;font-weight:900;color:#fff;letter-spacing:-.04em;margin-bottom:3px}
.fp-note{font-size:9px;color:rgba(255,255,255,.5)}
...
.fp-tog.on{background:rgba(141,198,63,.15);border-color:rgba(141,198,63,.35);color:var(--g3)}
...
.fp-btn{width:100%;background:var(--g3);color:#040D06;border:none;border-radius:9px;padding:12px;font-family:var(--font-display);font-size:11px;font-weight:800;cursor:pointer}
.fp2-body{padding:12px 12px 16px}
.fp2-avatar{width:36px;height:36px;border-radius:50%;background:rgba(141,198,63,.2);display:flex;align-items:center;justify-content:center;font-family:var(--font-display);font-size:12px;font-weight:900;color:var(--g3);margin:0 auto 7px}
...
.fp2-btn{width:100%;background:var(--g1);color:#fff;border:none;border-radius:9px;padding:10px;font-family:var(--font-display);font-size:10px;font-weight:800;cursor:pointer;margin-bottom:6px}
...
.ftag-val{font-family:var(--font-display);font-size:16px;font-weight:900;color:var(--g3);letter-spacing:-.03em}
.ftag-lbl{font-size:9px;color:rgba(255,255,255,.4);margin-top:1px}

/* ══════════════════════════════
   SECTION 4 — How it works (green bg)
══════════════════════════════ */
.how-section{background:var(--g1);padding:100px 5%;position:relative;overflow:hidden}
.how-bg-pattern{
...
.how-tag{display:inline-flex;align-items:center;gap:6px;background:rgba(255,255,255,.15);color:rgba(255,255,255,.9);font-size:12px;font-weight:500;padding:5px 14px;border-radius:20px;margin-bottom:16px}
...
.how-sub{font-size:17px;color:rgba(255,255,255,.6);max-width:420px;line-height:1.7;margin-bottom:60px}
...
.step-card:hover{background:rgba(255,255,255,.13);border-color:rgba(255,255,255,.25)}
.step-num{font-family:var(--font-display);font-size:11px;font-weight:800;color:var(--g3);letter-spacing:.1em;margin-bottom:18px;display:flex;align-items:center;gap:8px}
.step-num::after{content:'';flex:1;height:0.5px;background:rgba(255,255,255,.15)}
.step-icon{width:48px;height:48px;border-radius:14px;background:rgba(141,198,63,.2);border:0.5px solid rgba(141,198,63,.3);display:flex;align-items:center;justify-content:center;font-size:22px;color:var(--g3);margin-bottom:16px}
...
/* ══════════════════════════════
   SECTION 5 — Fee table (white)
══════════════════════════════ */
.fee-section{background:var(--white);padding:100px 5%}
.fee-center{text-align:center;max-width:600px;margin:0 auto}
.section-tag-green{display:inline-flex;align-items:center;gap:6px;background:var(--g-soft);color:var(--g1);font-size:12px;font-weight:500;padding:5px 14px;border-radius:20px;border:0.5px solid var(--border-g);margin-bottom:16px}
.section-h2{font-family:var(--font-display);font-size:clamp(28px,4vw,48px);font-weight:900;letter-spacing:-.05em;line-height:1.05;color:var(--ink);margin-bottom:14px}
...
.fee-head{display:grid;grid-template-columns:2fr 1fr 1fr;padding:14px 24px;background:var(--ink)}
.fee-head span{font-size:11px;font-weight:500;color:rgba(255,255,255,.6);text-transform:uppercase;letter-spacing:.07em}
...
.fee-row:hover{background:var(--g-lime)}
.fee-row.hi{background:var(--g-soft)}
.fee-method{font-size:14px;color:var(--ink2);display:flex;align-items:center;gap:7px}
.fee-them{font-size:14px;color:var(--ink3);text-align:center;text-decoration:line-through;opacity:.6}
.fee-us{font-size:14px;font-weight:500;color:var(--g1);text-align:center;display:flex;align-items:center;justify-content:center;gap:4px}

/* ══════════════════════════════
   SECTION 6 — Testimonials (deep)
══════════════════════════════ */
.testi-section{background:#040D06;padding:100px 5%;position:relative;overflow:hidden}
.testi-glow{position:absolute;pointer-events:none;border-radius:50%;filter:blur(100px);width:500px;height:300px;background:#1E6B32;opacity:.15;top:50%;left:50%;transform:translate(-50%,-50%)}
...
.testi-card:hover{border-color:rgba(141,198,63,.25)}
.testi-stars{display:flex;gap:3px;margin-bottom:14px}
.tstar{color:var(--g3);font-size:14px}
.testi-text{font-size:14px;color:rgba(255,255,255,.6);line-height:1.75;margin-bottom:20px;font-style:italic}
...
/* ══════════════════════════════
   SECTION 7 — App download (green)
══════════════════════════════ */
.app-section{
  background:var(--g1);padding:100px 5%;
  position:relative;overflow:hidden;
}
.app-bg-dots{position:absolute;inset:0;opacity:.08;background-image:radial-gradient(circle,rgba(255,255,255,.9) 1px,transparent 1px);background-size:24px 24px}
.app-glow{position:absolute;width:600px;height:600px;border-radius:50%;background:#8DC63F;filter:blur(120px);opacity:.12;bottom:-200px;right:-100px;pointer-events:none}
...
.app-eyebrow{font-size:11px;font-weight:500;color:var(--g3);letter-spacing:.1em;text-transform:uppercase;margin-bottom:16px}
...
.app-sub{font-size:16px;color:rgba(255,255,255,.6);line-height:1.7;margin-bottom:36px;max-width:380px}
...
.app-btn:hover{background:rgba(255,255,255,.2);transform:translateY(-2px)}
.app-btn-icon{font-size:26px;color:#fff}
.app-btn-small{font-size:10px;color:rgba(255,255,255,.55)}
.app-btn-big{font-family:var(--font-display);font-size:16px;font-weight:800;color:#fff;letter-spacing:-.02em}
.app-phone{
  background:#0D1F10;border-radius:36px;padding:8px;
  border:0.5px solid rgba(255,255,255,.1);
  box-shadow:0 32px 80px rgba(0,0,0,.4);
  max-width:240px;margin:0 auto;
}
.app-screen{background:#050E07;border-radius:30px;padding:16px 14px;min-height:300px}
...
.app-s-header span{color:var(--g3)}
.app-s-lbl{font-size:9px;color:rgba(255,255,255,.3);text-transform:uppercase;letter-spacing:.07em;font-weight:500;margin-bottom:8px}
.app-tx{display:flex;flex-direction:column;gap:7px}
.app-tx-item{background:rgba(255,255,255,.05);border-radius:10px;padding:9px 11px;display:flex;align-items:center;gap:9px}
...
.atx-note{font-size:9px;color:rgba(255,255,255,.35)}
.atx-amt{margin-left:auto;font-family:var(--font-display);font-size:12px;font-weight:800;color:var(--g3)}
.app-total{background:rgba(141,198,63,.12);border-radius:10px;padding:10px 12px;display:flex;justify-content:space-between;align-items:center;margin-top:8px}
.atotal-lbl{font-size:9px;color:rgba(255,255,255,.4)}
.atotal-val{font-family:var(--font-display);font-size:16px;font-weight:900;color:var(--g3)}

/* ══════════════════════════════
   SECTION 8 — Trust (white)
══════════════════════════════ */
.trust-section{background:var(--white);padding:72px 5%;text-align:center}
.trust-h3{font-family:var(--font-display);font-size:14px;font-weight:700;color:var(--ink3);letter-spacing:.05em;text-transform:uppercase;margin-bottom:24px}
...
.trust-pill:hover{background:var(--g-soft);border-color:var(--border-g);color:var(--g1)}
.tp-dot{width:7px;height:7px;border-radius:50%;background:var(--g3)}

/* ══════════════════════════════
   FINAL CTA (deep)
══════════════════════════════ */
.final-section{
  background:#040D06;padding:120px 5%;text-align:center;
  position:relative;overflow:hidden;
}
.final-glow{position:absolute;width:600px;height:400px;border-radius:50%;background:var(--g1);filter:blur(100px);opacity:.25;top:50%;left:50%;transform:translate(-50%,-50%);pointer-events:none}
...
.final-h2 em{color:var(--g3);font-style:normal}
.final-sub{font-size:18px;color:rgba(255,255,255,.45);margin-bottom:40px}
...
.btn-final-fill:hover{background:var(--g4);transform:translateY(-2px)}
.btn-final-ghost{
  display:inline-flex;align-items:center;gap:8px;
  background:rgba(255,255,255,.08);color:rgba(255,255,255,.7);
  padding:16px 32px;border-radius:100px;
  font-family:var(--font-body);font-size:16px;font-weight:500;
  cursor:pointer;border:0.5px solid rgba(255,255,255,.15);text-decoration:none;
  transition:all .2s;backdrop-filter:blur(8px);
}
.btn-final-ghost:hover{background:rgba(255,255,255,.14)}
...
.footer-col-title{font-size:11px;font-weight:500;color:rgba(255,255,255,.35);text-transform:uppercase;letter-spacing:.08em;margin-bottom:14px}
.footer-links{display:flex;flex-direction:column;gap:9px}
.footer-links a{font-size:13px;color:rgba(255,255,255,.4);text-decoration:none;transition:color .15s}
.footer-links a:hover{color:#fff}
.footer-bottom{border-top:0.5px solid rgba(255,255,255,.07);padding-top:24px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px}
.footer-copy{font-size:12px;color:rgba(255,255,255,.25)}
.footer-arc{font-size:12px;color:rgba(255,255,255,.25);display:flex;align-items:center;gap:5px}
.footer-arc span{color:var(--g3)}
...
@media(max-width:900px){
  ...
  .fp-secondary{border:10px solid #2a2a2a;outline:1.5px solid #444;box-shadow:0 0 0 1.5px #555,inset 0 0 0 1px #111,0 30px 60px rgba(0,0,0,.7);border:8px solid #1a1a1a;box-shadow:0 0 0 1px #333,0 30px 60px rgba(0,0,0,.7),inset 0 0 0 1px #444;display:none}
}
...
` }} />
  ...
  {/* TESTNET BANNER */}
<div id="testnet-banner" style={{ display: bannerOpen ? "flex" : "none", background:"#1E6B32",color:"#fff",padding:"8px 20px",alignItems:"center",justifyContent:"center",gap:10,fontSize:13,fontWeight:500,position:"relative",zIndex:1000 }}>
  <span style={{"width":"7px","height":"7px","borderRadius":"50%","background":"#8DC63F","display":"inline-block","flexShrink":0}}></span>
  PayLink is live on Arc Testnet — all transactions use test funds.
  <a href="https://testnet.arcscan.app" target="_blank" style={{"color":"#8DC63F","textDecoration":"underline"}}>View explorer</a>
  <button onClick={() => setBannerOpen(false)} style={{"background":"none","border":"none","color":"rgba(255,255,255,0.6)","cursor":"pointer","fontSize":"16px","position":"absolute","right":"16px"}}>×</button>
</div>
  ...
  <div className="nav-right">
    {authenticated ? (
      <div style={{display:'flex',alignItems:'center',gap:10}}>
        <button onClick={() => router.push('/dashboard')} className="btn-pill btn-dark" style={{cursor:'pointer',border:'none'}}>Dashboard</button>
      </div>
    ) : (
      <span style={{display:'flex',alignItems:'center',gap:10}}>
        <button onClick={login} className="btn-pill btn-ghost" style={{borderColor:'var(--border-g)',color:'var(--g1)',cursor:'pointer'}}>Sign in</button>
        <button onClick={() => router.push('/create')} className="btn-pill btn-dark" style={{cursor:'pointer',border:'none'}}>Create link</button>
      </span>
    )}
  </div>
</nav>
...
<section className="hero">
  ...
  <div className="hero-inner">
    ...
    <div className="hero-ctas">
      {authenticated ? (
        <button onClick={() => router.push('/dashboard')} className="btn-hero-dark" style={{cursor:"pointer",border:"none"}}>
          <iconify-icon icon="ph:squares-four-bold"></iconify-icon>
          Go to Dashboard
        </button>
      ) : (
        <>
          <button onClick={() => router.push('/send')} className="btn-hero-dark" style={{cursor:"pointer",border:"none"}}>
            <iconify-icon icon="ph:paper-plane-right-bold"></iconify-icon>
            Send money
          </button>
          <button onClick={() => router.push('/create')} className="btn-hero-light" style={{cursor:"pointer"}}>
...
<section className="deep-section">
  ...
  <div className="deep-inner">
    ...
    <div className="float-grid">
      <div className="glass-card" style={{"transitionDelay":".05s"}}>
        <div className="gc-icon"><iconify-icon icon="ph:user-plus-bold"></iconify-icon></div>
        <div className="gc-title">No account to pay</div>
        <div className="gc-desc">Senders click the link and pay. No signup. No wallet. No app download. Works on any browser worldwide.</div>
      </div>
      <div className="glass-card" style={{"transitionDelay":".15s"}}>
        <div className="gc-icon"><iconify-icon icon="ph:lightning-bold"></iconify-icon></div>
        <div className="gc-title">Sub-second settlement</div>
        <div className="gc-desc">Arc confirms payments in under one second. No "pending" for 3 business days. The moment they pay, you have it.</div>
      </div>
      <div className="glass-card" style={{"transitionDelay":".25s"}}>
        <div className="gc-icon"><iconify-icon icon="ph:currency-circle-dollar-bold"></iconify-icon></div>
        <div className="gc-title">Any currency, any direction</div>
        <div className="gc-desc">Sender pays in NGN, GBP, USD or USDC. Receiver gets local fiat or crypto. Seamlessly, automatically.</div>
      </div>
      <div className="glass-card" style={{"transitionDelay":".35s"}}>
        <div className="gc-icon"><iconify-icon icon="ph:globe-bold"></iconify-icon></div>
        <div className="gc-title">Truly global coverage</div>
        <div className="gc-desc">Ramp Network for 150+ countries. Yellow Card for deep Africa — mobile money, bank transfers, all of it.</div>
      </div>
      <div className="glass-card" style={{"transitionDelay":".45s"}}>
        <div className="gc-icon"><iconify-icon icon="ph:shield-check-bold"></iconify-icon></div>
        <div className="gc-title">Non-custodial</div>
        <div className="gc-desc">We never hold your funds. Every transaction settles directly on Arc — transparent, trustless, verifiable on-chain.</div>
      </div>
      <div className="glass-card" style={{"transitionDelay":".55s"}}>
        <div className="gc-icon"><iconify-icon icon="ph:repeat-bold"></iconify-icon></div>
        <div className="gc-title">Links that never expire</div>
        <div className="gc-desc">Put your PayLink in your Instagram bio and get paid forever. One link, unlimited payments, zero maintenance.</div>
      </div>
    </div>
  </div>
</section>
...
<section id="how" className="how-section">
  ...
  <div className="how-inner">
    ...
    <div className="steps-grid">
      <div className="step-card" style={{"transitionDelay":".05s"}}>
        <div className="step-num">STEP 01</div>
        <div className="step-icon"><iconify-icon icon="ph:link-bold"></iconify-icon></div>
        <div className="step-title">Create your link</div>
        <div className="step-desc">Enter an amount, add a note, choose how to receive — crypto wallet or bank account. Your link is ready in 30 seconds.</div>
      </div>
      <div className="step-card" style={{"transitionDelay":".15s"}}>
        <div className="step-num">STEP 02</div>
        <div className="step-icon"><iconify-icon icon="ph:share-network-bold"></iconify-icon></div>
        <div className="step-title">Share it anywhere</div>
        <div className="step-desc">Send via WhatsApp, Telegram, X, or paste in your bio. Anyone with the link can pay — no app, no account needed.</div>
      </div>
      <div className="step-card" style={{"transitionDelay":".25s"}}>
        <div className="step-num">STEP 03</div>
        <div className="step-icon"><iconify-icon icon="ph:lightning-bold"></iconify-icon></div>
        <div className="step-title">Get paid instantly</div>
        <div className="step-desc">Settlement in under a second on Arc. Receive USDC in your wallet or local currency straight to your bank or mobile money.</div>
      </div>
      <div className="step-card" style={{"transitionDelay":".35s"}}>
        <div className="step-num">BONUS</div>
        <div className="step-icon"><iconify-icon icon="ph:repeat-bold"></iconify-icon></div>
        <div className="step-title">Reuse forever</div>
        <div className="step-desc">Your PayLink never expires unless you set it to. One link in your bio, unlimited payments, zero maintenance.</div>
      </div>
    </div>
  </div>
</section>
...
<section id="fees" className="fee-section">
  <div className="fee-center reveal">
    <div className="section-tag-green"><iconify-icon icon="ph:percent-bold"></iconify-icon>Transparent fees</div>
    ...
    <div className="testi-grid">
      <div className="testi-card" style={{"transitionDelay":".05s"}}>
        <div className="testi-stars">
          <iconify-icon icon="ph:star-fill" className="tstar"></iconify-icon><iconify-icon icon="ph:star-fill" className="tstar"></iconify-icon><iconify-icon icon="ph:star-fill" className="tstar"></iconify-icon><iconify-icon icon="ph:star-fill" className="tstar"></iconify-icon><iconify-icon icon="ph:star-fill" className="tstar"></iconify-icon>
        </div>
        ...
        <div className="testi-author">
          <div className="tav" style={{"background":"rgba(30,107,50,.3)","color":"#8DC63F"}}>AO</div>
      ...
      <div className="testi-card" style={{"transitionDelay":".15s"}}>
        ...
        <div className="testi-author">
          <div className="tav" style={{"background":"rgba(57,73,171,.3)","color":"#8090FF"}}>JM</div>
      ...
      <div className="testi-card" style={{"transitionDelay":".25s"}}>
        ...
        <div className="testi-author">
          <div className="tav" style={{"background":"rgba(194,24,91,.2)","color":"#FF80AB"}}>FC</div>
      ...
      <div className="app-phone">
        <div className="app-screen">
          ...
          <div className="app-tx">
            <div className="app-tx-item">
              <div className="atav" style={{"background":"rgba(30,107,50,.25)","color":"#8DC63F"}}>JK</div>
              <div><div className="atx-name">James K.</div><div className="atx-note">Website redesign</div></div>
              <div className="atx-amt">+$500</div>
            </div>
            <div className="app-tx-item">
              <div className="atav" style={{"background":"rgba(255,180,0,.15)","color":"#FFB400"}}>AM</div>
              <div><div className="atx-name">Amara M.</div><div className="atx-note">Consulting fee</div></div>
              <div className="atx-amt">+$150</div>
            </div>
            <div className="app-tx-item">
              <div className="atav" style={{"background":"rgba(100,130,255,.15)","color":"#8090FF"}}>TL</div>
              <div><div className="atx-name">Tolu L.</div><div className="atx-note">April retainer</div></div>
              <div className="atx-amt">+$800</div>
            </div>
          </div>
...
<section className="trust-section reveal">
  ...
  <div className="trust-pills">
    <div className="trust-pill"><div className="tp-dot"></div>Arc Network</div>
    <div className="trust-pill"><div className="tp-dot"></div>Circle USDC</div>
    <div className="trust-pill"><div className="tp-dot"></div>Privy Wallets</div>
    <div className="trust-pill"><div className="tp-dot"></div>Pimlico</div>
    <div className="trust-pill"><div className="tp-dot"></div>Ramp Network</div>
    <div className="trust-pill"><div className="tp-dot"></div>Yellow Card</div>
  </div>
</section>
